...
export default function MembersTable({ members, loading, onRefresh }) {
  ...
  const getRoleBadge = (role) => {
    const roleConfig = {
      ...
      member: { color: "bg-blue-100 text-[#4b2c5e]", label: "Membre" },
    };
        ...
        <div className="flex items-center space-x-2">
          ...
          {selectedProfession !== "all" && (
            <button
              ...
              className="text-sm text-blue-600 hover:text-[#4b2c5e] hover:underline"
            >
              Réinitialiser
            </button>
          )}
            ...
            <div className="text-sm text-gray-700">
              ...
              {selectedProfession !== "all" && (
                <>
                  {" "}
                  pour la profession:{" "}
                  <span className="font-medium">{selectedProfession}</span>
                </>
              )}
            </div>
            {selectedProfession !== "all" && (
        ...
        {filteredMembers.length === 0 ? (
          <div className="text-center py-8">
            ...
            {selectedProfession !== "all" && (
              <button
                onClick={() => setSelectedProfession("all")}
                className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-[#4b2c5e] transition-colors"
              >
