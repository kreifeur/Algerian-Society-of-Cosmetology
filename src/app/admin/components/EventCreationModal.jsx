...
export default function EventCreationModal({ onClose, onEventCreated }) {
  ...
  const handleDeleteCategory = async (categoryId) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "Erreur lors de la suppression de la catégorie",
        );
      }
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      const createdCategory =
        categories.find((cat) => cat.name === newCategoryName) ||
        categories.find(
          (cat) =>
            cat._id === (result.data?.category?._id || result.category?._id),
        );
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      const selectedCategory = categories.find(
        (cat) =>
          cat._id === eventForm.category || cat.id === eventForm.category,
      );
          ...
          <form onSubmit={handleCreateEvent} className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button
                    ...
                    className="text-sm text-blue-600 hover:text-[#4b2c5e]"
                    disabled={eventLoading}
                  >
                    ...
                    {!loadingCategories && categories.length > 0 && (
                      <div className="mt-3">
                        <button
                          ...
                          className="flex items-center text-sm text-gray-600 hover:text-[#4b2c5e]"
                        >
                          <svg
                              ...
                              {categories.map((category) => (
                                ...
                                >
                                  <div className="flex items-center">
                                    <span
                                      className={`text-sm ${eventForm.category === category._id ? "text-blue-600 font-medium" : "text-gray-700"}`}
                                    >
                                      {category.name}
                                      {eventForm.category === category._id &&
                                        " (sélectionnée)"}
                                    </span>
                                  </div>
                                  <button
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <button
                type="submit"
                disabled={eventLoading}
                className="px-4 py-2 bg-[#4b2c5e] text-white rounded-md hover:bg-[#4b2c5e] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center space-x-2"
              >
                {eventLoading && (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600 mb-6">
                Cette action est irréversible. Les événements utilisant cette
                catégorie pourront être affectés.
              </p>
