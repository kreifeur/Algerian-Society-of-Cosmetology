...
export default function ArticlesTable({ articles, loading, onRefresh }) {
  ...
  const getAccessBadge = (isMemberOnly) => {
    return isMemberOnly ? (
      ...
      <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-100 text-[#4b2c5e]">
        Public
      </span>
    );
  };
  ...
  const handleUpdateArticle = async (articleId) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "Erreur lors de la mise à jour de l'article",
        );
      }
  ...
  const handleDeleteArticle = async (articleId) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "Erreur lors de la suppression de l'article",
        );
      }
  ...
  const handleTogglePublish = async (articleId, currentStatus) => {
    try {
      ...
      if (result.success) {
        setArticleMessage(
          `✅ Article ${newStatus ? "publié" : "dépublié"} avec succès !`,
        );
        onRefresh();
        setTimeout(() => setArticleMessage(""), 2000);
      } else {
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <button
                              ...
                              className="text-blue-600 hover:text-[#4b2c5e]"
                            >
                              Modifier
                            </button>
                            {/*                             <button
