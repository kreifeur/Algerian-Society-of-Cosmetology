          ...
          xhr.addEventListener("load", () => {
            if (xhr.status === 200) {
              ...
              try {
                ...
                reject(
                  new Error(errorResponse.error?.message || "Upload échoué"),
                );
              } catch {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-[#4b2c5e]">
          ...
          {loading && uploadProgress > 0 && uploadProgress < 100 && (
            <div className="mb-6 p-4 bg-[#ddd3e6] border border-blue-200 rounded-md">
          ...
          <form onSubmit={handleSubmit}>
                  ...
                  {formData.tags.length > 0 ? (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {formData.tags.map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-[#4b2c5e]"
                        >
                          {tag}
                          <button
                            type="button"
                            onClick={() => handleRemoveTag(tag)}
                            className="ml-2 text-blue-600 hover:text-[#4b2c5e] text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={loading}
                          >
                            ×
                          </button>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end space-x-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-4 py-2 bg-[#4b2c5e] text-white rounded-md hover:bg-[#4b2c5e] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2 min-w-[150px] justify-center"
              >
                {loading ? (
                  <>
                    <svg
