...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    const fetchGallery = async () => {
      try {
        ...
        if (result.success) {
          ...
          const allItems = (result.data.media || []).map((item) => {
            ...
            if (item.fileType) {
              ...
              item.tags &&
              (item.tags.includes("video") || item.tags.includes("vidéo"))
            ) {
              detectedType = "video";
            }
            ...
            if (item.tags) {
              ...
              if (
                tags.some(
                  (tag) => tag.includes("workshop") || tag.includes("atelier"),
                )
              ) {
                category = "workshops";
              } else if (
                tags.some(
                  (tag) => tag.includes("visit") || tag.includes("visite"),
                )
              ) {
                category = "visits";
              } else if (
                tags.some(
                  (tag) => tag.includes("video") || tag.includes("vidéo"),
                )
              ) {
                category = "videos";
              } else if (
                tags.some(
                  (tag) => tag.includes("event") || tag.includes("événement"),
                )
              ) {
                category = "events";
              }
            ...
            return {
              ...
              thumbnailUrl:
                item.thumbnailUrl || item.url || `/gallery-placeholder.jpg`,
              tags: item.tags || "",
              title:
                item.title ||
                `Événement ${Math.floor(Math.random() * 100) + 1}`,
          ...
          if (!token) {
            ...
            filteredItems = allItems.filter(
              (item) => item.isMemberOnly !== true,
            );
            console.log(
              "Non-authenticated user: Showing",
            ...
            console.log(
              "Authenticated user: Showing all",
              filteredItems.length,
              "items",
            );
          }
        ...
        if (!token) {
          filteredFallback = fallbackItems.filter(
            (item) => item.isMemberOnly !== true,
          );
        }
  ...
  return (
    <div>
      <Head>
        <title>Galerie - Algerian Society of Cosmetology</title>
        <meta
          name="description"
          content="Galerie photos et vidéos des événements de l'Association de Cosmétologie"
        />
      ...
      <main className="min-h-screen bg-[#ddd3e6] py-12">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-center text-[#4b2c5e] mb-4">
            Galerie
          </h1>
          ...
          {isLoading && (
            <div className="text-center py-12">
              <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-bg-[#4b2c5e]"></div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full transition-colors ${
                    activeFilter === filter.key
                      ...
                      : "bg-white text-[#4b2c5e] hover:bg-blue-100"
                  }`}
                >
                  {filter.label}
                </button>
                  ...
                  >
                    ...
                    {item.isMemberOnly && (
                      <div className="absolute top-2 right-2 z-10">
                        <span className="bg-[#4b2c5e] text-white text-xs px-2 py-1 rounded-full flex items-center">
                      ...
                      {item.type === "video" ? (
                        <>
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 group-hover:bg-opacity-10 transition-opacity">
                            <div className="bg-[#4b2c5e] bg-opacity-80 rounded-full p-4 group-hover:scale-110 transition-transform">
                              <svg
                    ...
                    <div className="p-4">
                      <h3 className="font-semibold text-[#4b2c5e] mb-1 truncate">
                        {item.title}
                      </h3>
                      ...
                      <div className="flex justify-between items-center mt-3">
                        <span className="text-xs px-2 py-1 bg-blue-100 text-[#4b2c5e] rounded-full capitalize truncate max-w-[120px]">
                          {Array.isArray(item.tags)
                            ? item.tags[0]
                  ...
                  {galleryItems.length === 0 ? (
                    <div>
                      ...
                      {!authToken && (
                        <a
                          href="/login"
                          className="inline-block px-6 py-3 bg-[#4b2c5e] text-white rounded-md hover:bg-[#4b2c5e] transition-colors font-medium"
                        >
                          Se connecter
                        </a>
                      )}
          ...
          {lightboxOpen && filteredItems[currentImage] && (
            ...
            >
              ...
              <button
                ...
                className="absolute left-4 text-white text-3xl z-10 bg-[#4b2c5e] bg-opacity-80 rounded-full h-12 w-12 flex items-center justify-center hover:bg-opacity-100 transition-all hover:scale-110"
              ...
              <button
                onClick={goToNext}
                className="absolute right-4 text-white text-3xl z-10 bg-[#4b2c5e] bg-opacity-80 rounded-full h-12 w-12 flex items-center justify-center hover:bg-opacity-100 transition-all hover:scale-110"
                aria-label="Suivant"
              >
                ›
              </button>
              ...
              <div className="max-w-5xl w-full max-h-[90vh] bg-gray-900 rounded-lg overflow-hidden shadow-2xl">
                <div className="h-[70vh] bg-black flex items-center justify-center">
                  {filteredItems[currentImage].type === "image" ? (
                    <img
                      src={
                        filteredItems[currentImage].thumbnailUrl ||
                        filteredItems[currentImage].thumbnailUrl
                      }
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      ...
                      >
                        <source
                          src={filteredItems[currentImage].thumbnailUrl}
                          type="video/mp4"
                        />
                        <source
                          src={filteredItems[currentImage].thumbnailUrl}
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-4">
                    <div className="flex items-center gap-2">
                        ...
                        {Array.isArray(filteredItems[currentImage].tags)
                          ? filteredItems[currentImage].tags.map((tag, i) => (
                              <span key={i} className="mr-1">
                                {tag}
                                {i < filteredItems[currentImage].tags.length - 1
                                  ? ","
                                  : ""}
                              </span>
                            ))
                      ...
                      <span className="text-sm px-3 py-1 bg-gray-700 rounded-full">
                        {filteredItems[currentImage].type === "video"
                          ? "🎬 Vidéo"
                          : "📷 Photo"}
                      </span>
                    </div>
          ...
          {/* <section className="mt-16">
            <h2 className="text-3xl font-bold text-center text-[#4b2c5e] mb-12">Ils témoignent</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center mb-4">
                  <div className="h-12 w-12 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                    <span className="text-[#4b2c5e] font-semibold">ML</span>
                  </div>
                  <div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center mb-4">
                  <div className="h-12 w-12 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                    <span className="text-[#4b2c5e] font-semibold">PD</span>
                  </div>
                  <div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center mb-4">
                  <div className="h-12 w-12 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                    <span className="text-[#4b2c5e] font-semibold">SC</span>
                  </div>
                  <div>
          ...
          {/* Newsletter */}
          <section className="mt-16 bg-[#4b2c5e] rounded-lg p-8 text-center text-white">
            <h2 className="text-2xl font-semibold mb-4">Restez informé</h2>
            <p className="mb-6 max-w-2xl mx-auto">
              Inscrivez-vous à notre newsletter pour recevoir les actualités de
              l'association, les invitations à nos événements et les dernières
            ...
            <form className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
              ...
              <button
                type="submit"
                className="px-6 py-2 bg-yellow-500 text-[#4b2c5e] rounded-md hover:bg-yellow-400 transition-colors font-medium"
              >
                S'inscrire
              </button>
            </form>
